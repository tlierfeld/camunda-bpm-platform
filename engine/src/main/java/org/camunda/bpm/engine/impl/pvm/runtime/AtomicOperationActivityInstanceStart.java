...
public abstract class AtomicOperationActivityInstanceStart extends AbstractEventAtomicOperation {
  ...
  protected InterpretableExecution eventNotificationsStarted(InterpretableExecution execution) {
    ...
    if(activity.isInterrupting()) {

      // find scope execution:
      InterpretableExecution scopeExecution = (InterpretableExecution) (execution.isScope() ? execution : execution.getParent());

      // create new activity instance before destroying the scope
      scopeExecution.enterActivityInstance();
      scopeExecution.destroyScope("Interrupting activity "+activity+" executed.");
      propagatingExecution = scopeExecution;
